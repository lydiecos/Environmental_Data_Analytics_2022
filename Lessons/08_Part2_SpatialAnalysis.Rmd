...
#Create a map
mapView(huc8_sf_filtered)
ggplot() +
  geom_sf(data = huc8_sf, fill = "orange") +
  geom_sf(data = huc8_sf_filtered, fill = "purple")
...
wind_nc_raw <- read.csv(
  ...
  group_by(HUC_8)  %>%
  summarize(SunWindEnergy = sum(AvgWindEnergy))

#Join to HUC_8 features
wind_huc_joined <- huc8_sf %>%
  left_join(wind_nc_raw)



#View the outputs
ggplot(wind_huc_joined) +
  geom_sf(aes(fill = SunWindEnergy)) +
...
```{r EXERCISE: Clipping}
#Select the Triangle County from the rest
...
triangle_HUCs_clipped <- triangle_co %>%
  st_intersection(huc8_sf_utm)
mapview(triangle_HUCs_clipped)
