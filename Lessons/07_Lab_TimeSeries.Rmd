...
rain.ts <- ts(BRA_complete$Rainfall, start = c(fyear, fmonth), frequency = 12)

temp.ts <- ts(BRA_complete$Temperature_C, start = c(fyear, fmonth), frequency = 12)
...
rain.ts.decomposed <- stl(rain.ts, s.window = "periodic")
temp.ts.decomposed <- stl(temp.ts, s.window = "periodic")

plot(rain.ts.decomposed)
plot(temp.ts.decomposed)
...
rain.ts.components <- as.data.frame(rain.ts.decomposed$time.series[,1:3])
temp.ts.components <- as.data.frame(temp.ts.decomposed$time.series[,1:3])


# Exercise 10: Visualize how the trend maps onto the data for both series
rain.ts.components <- mutate(rain.ts.components,
                             Observed = BRA_complete$Rainfall,
                             Date = BRA_complete$Date)
temp.ts.components <- mutate(temp.ts.components,
                             Observed = BRA_complete$Temperature_C,
                             Date = BRA_complete$Date)

ggplot(rain.ts.components, aes(x=Date)) +
  geom_line(aes(y=Observed)) +
  geom_line(aes(y=trend), color="Red") +
  theme_bw()
  
ggplot(temp.ts.components, aes(x=Date)) +
  geom_line(aes(y=Observed)) +
  geom_line(aes(y=trend), color="Red") +
  theme_bw()
...
Kendall::SeasonalMannKendall(rain.ts)
