...
> Answer: Depends on the packaged, but shows any conflicts, e.g., dplyr and stats both have a "filter()" function. 
...
> Answer: Remove if it's happenstance that the data isn't there (e.g., it just wasn't collected), but leave in if the thing was actually measured - e.g., measuring plant height but some plants have died so don't have a height.
